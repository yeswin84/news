...
신문 기사, 토론 음성, 텍스트 메모를 바탕으로 요약, 핵심 포인트, 개념 정리, 내 메모를 한 번에 만드는 개인용 웹앱입니다. 여러 음성 파일을 한 기록으로 묶어 처리하고, 노션용 복사와 PDF/Markdown 저장까지 지원합니다.

## 반복 배포

수정 후 GitHub와 Vercel에 함께 반영하려면 `publish_reading_note.bat`를 실행합니다.

첫 실행 때 GitHub 저장소 URL을 입력하면 이후에는 같은 원격 저장소로 커밋, 푸시, Vercel 프로덕션 배포까지 이어집니다.
